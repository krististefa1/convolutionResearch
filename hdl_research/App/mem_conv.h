...
static const char kernel_hardcode[9] = {
    0x02, 0x0B, 0x02,
    0x05, 0x0E, 0x02,
    0x02, 0x0B, 0x02
};
...
struct window {
	unsigned char pix[KERNEL_DIM][KERNEL_DIM];
};


FRESULT timePrint();
void filter2d_SW(u8* img_in,char* kernel, u8* img_out, int rows, int cols);
void Window2D(int width, int height, u8* img_in, u32* img_out);
void saveBuffer(u32* dst, int numBuffers, int numOfChunks);
